...
async function seedData(db) {
  ...
  // Community Profiles
  const profiles = [
    ['cp-1','Somali Community','East African',JSON.stringify(['Somali','English','Arabic']),'Strong oral tradition, clan-based social structure, Islamic faith central to daily life','Formal introductions matter. Allow time for relationship building before business. Gender considerations in service delivery.','Trust built through community elders and mosques. Fear of government involvement due to refugee experiences.'],
    ['cp-2','Hmong Community','Southeast Asian',JSON.stringify(['Hmong','English']),'Clan-based structure with strong family loyalty. Elder respect is paramount.','Use trained interpreters — not family members. Allow extra time. Written materials less effective than in-person.','Trust through clan leaders and Hmong community organizations. Historical trauma from war displacement.'],
    ['cp-3','Indigenous Nations','Indigenous',JSON.stringify(['Ojibwe','Dakota','English']),'Sovereign nations with distinct cultural practices, languages, and governance structures.','Recognize tribal sovereignty. Use proper nation names. Allow time for consensus-based decision making.','Trust through tribal social service departments and tribal councils. Historical trauma from boarding schools and forced assimilation.'],
    ['cp-4','Latino/a/x Community','Latin American',JSON.stringify(['Spanish','English']),'Diverse community with significant variation by country of origin, generation, and documentation status.','Spanish language materials essential. Familismo — family involvement in decisions. Personalismo — relationship first.','Trust through promotores, churches, and Latino-serving nonprofits. Fear of immigration enforcement.'],
    ['cp-5','Deaf and Hard of Hearing','Disability',JSON.stringify(['ASL','English']),'Deaf community has distinct cultural identity. ASL is a complete language, not a form of English.','Always provide ASL interpreters. Do not speak to the interpreter — speak to the Deaf person. Written notes insufficient.','Trust through Deaf community centers and Deaf-led organizations.'],
    ['cp-6','East African Community','East African',JSON.stringify(['Amharic','Tigrinya','Oromo','Somali','English']),'Diverse community from Ethiopia, Eritrea, and other East African countries.','Multiple distinct ethnic and language groups within this category. Avoid treating as monolithic.','Trust through Ethiopian and Eritrean community organizations.'],
  ];
  profiles.forEach(p => db.run(`INSERT OR IGNORE INTO community_profiles (id,community_name,category,languages_json,cultural_context,communication_guidance,trust_factors,priority_flag,created_at) VALUES (?,?,?,?,?,?,?,1,datetime('now'))`, p));
