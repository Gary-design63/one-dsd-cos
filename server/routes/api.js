...
function auditLog(db, eventType, userId, details) {
  try { db.run(`INSERT INTO audit_log (id,event_type,details,user_id,created_at) VALUES (?,?,?,?,datetime('now'))`, [uuidv4(),eventType,details,userId]); } catch(e){}
}
...
router.get('/equity-reviews', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM equity_reviews ORDER BY created_at DESC')); } catch(err){res.status(500).json({error:err.message});}
});
router.post('/equity-reviews', requireAuth, async (req, res) => {
  try {
    ...
    saveDb(); res.status(201).json(dbRow(db,'SELECT * FROM equity_reviews WHERE id=?',[id]));
  } catch(err){res.status(500).json({error:err.message});}
});
router.get('/equity-reviews/:id', requireAuth, async (req, res) => {
  try { const db=await getDb(); const r=dbRow(db,'SELECT * FROM equity_reviews WHERE id=?',[req.params.id]); r?res.json(r):res.status(404).json({error:'Not found'}); } catch(err){res.status(500).json({error:err.message});}
});
router.patch('/equity-reviews/:id', requireAuth, async (req, res) => {
  try {
    ...
    saveDb(); res.json(dbRow(db,'SELECT * FROM equity_reviews WHERE id=?',[req.params.id]));
  } catch(err){res.status(500).json({error:err.message});}
});
...
router.get('/action-items', requireAuth, async (req, res) => {
  try {
    ...
    res.json(dbRows(db,sql+' ORDER BY due_date ASC',p));
  } catch(err){res.status(500).json({error:err.message});}
});
router.post('/action-items', requireAuth, async (req, res) => {
  try {
    ...
    saveDb(); res.status(201).json(dbRow(db,'SELECT * FROM action_items WHERE id=?',[id]));
  } catch(err){res.status(500).json({error:err.message});}
});
router.patch('/action-items/:id', requireAuth, async (req, res) => {
  try {
    ...
    saveDb(); res.json(dbRow(db,'SELECT * FROM action_items WHERE id=?',[req.params.id]));
  } catch(err){res.status(500).json({error:err.message});}
});
...
router.get('/consultations', requireAuth, async (req, res) => {
  try {
    ...
    if(status){sql+=' AND status=?';p.push(status);} if(priority){sql+=' AND priority=?';p.push(priority);}
    res.json(dbRows(db,sql+" ORDER BY CASE priority WHEN 'urgent' THEN 1 WHEN 'high' THEN 2 ELSE 3 END, created_at DESC",p));
  } catch(err){res.status(500).json({error:err.message});}
});
router.post('/consultations', requireAuth, async (req, res) => {
  try {
    ...
    saveDb(); res.status(201).json(dbRow(db,'SELECT * FROM consultations WHERE id=?',[id]));
  } catch(err){res.status(500).json({error:err.message});}
});
router.patch('/consultations/:id', requireAuth, async (req, res) => {
  try {
    ...
    saveDb(); res.json(dbRow(db,'SELECT * FROM consultations WHERE id=?',[req.params.id]));
  } catch(err){res.status(500).json({error:err.message});}
});
...
router.get('/resources', requireAuth, async (req, res) => {
  try {
    ...
    res.json(dbRows(db,sql+' ORDER BY title ASC',p));
  } catch(err){res.status(500).json({error:err.message});}
});
router.post('/resources', requireAuth, requireRole('equity_lead'), async (req, res) => {
  try {
    ...
    saveDb(); res.status(201).json(dbRow(db,'SELECT * FROM resources WHERE id=?',[id]));
  } catch(err){res.status(500).json({error:err.message});}
});

// Community Profiles
router.get('/community-profiles', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM community_profiles ORDER BY community_name ASC')); } catch(err){res.status(500).json({error:err.message});}
});
router.get('/community-profiles/:id', requireAuth, async (req, res) => {
  try { const db=await getDb(); const r=dbRow(db,'SELECT * FROM community_profiles WHERE id=?',[req.params.id]); r?res.json(r):res.status(404).json({error:'Not found'}); } catch(err){res.status(500).json({error:err.message});}
});
router.patch('/community-profiles/:id', requireAuth, async (req, res) => {
  try {
    const db=await getDb(); const {cultural_context,communication_guidance,trust_factors,service_considerations}=req.body;
    db.run(`UPDATE community_profiles SET cultural_context=COALESCE(?,cultural_context),communication_guidance=COALESCE(?,communication_guidance),trust_factors=COALESCE(?,trust_factors),service_considerations=COALESCE(?,service_considerations) WHERE id=?`,
      [cultural_context??null,communication_guidance??null,trust_factors??null,service_considerations??null,req.params.id]);
    saveDb(); res.json(dbRow(db,'SELECT * FROM community_profiles WHERE id=?',[req.params.id]));
  } catch(err){res.status(500).json({error:err.message});}
});

// Training
router.get('/training-courses', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM training_courses ORDER BY level ASC, title ASC')); } catch(err){res.status(500).json({error:err.message});}
});
router.get('/training-progress', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM training_progress WHERE user_id=?',[req.user.id])); } catch(err){res.status(500).json({error:err.message});}
});
router.post('/training-progress', requireAuth, async (req, res) => {
  try {
    ...
    saveDb(); res.json(dbRow(db,'SELECT * FROM training_progress WHERE user_id=? AND course_id=?',[req.user.id,course_id]));
  } catch(err){res.status(500).json({error:err.message});}
});

// DEIA Topics
router.get('/deia-topics', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM deia_topics ORDER BY title ASC')); } catch(err){res.status(500).json({error:err.message});}
});

// Reflections
router.get('/reflections', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM reflections WHERE user_id=? ORDER BY created_at DESC',[req.user.id])); } catch(err){res.status(500).json({error:err.message});}
});
router.post('/reflections', requireAuth, async (req, res) => {
  try {
    ...
    saveDb(); res.status(201).json(dbRow(db,'SELECT * FROM reflections WHERE id=?',[id]));
  } catch(err){res.status(500).json({error:err.message});}
});

// Spaced Retrieval
router.get('/spaced-retrieval/this-week', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM spaced_retrieval_prompts WHERE active=1 ORDER BY RANDOM() LIMIT 3')); } catch(err){res.status(500).json({error:err.message});}
});

// Weekly Syntheses
router.get('/weekly-syntheses', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM weekly_syntheses ORDER BY week_start DESC LIMIT 12')); } catch(err){res.status(500).json({error:err.message});}
});

// Operational Goals
router.get('/operational-goals', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM operational_goals ORDER BY number ASC')); } catch(err){res.status(500).json({error:err.message});}
});
router.post('/operational-goals', requireAuth, requireRole('equity_lead'), async (req, res) => {
  try {
    ...
    saveDb(); res.status(201).json(dbRow(db,'SELECT * FROM operational_goals WHERE id=?',[id]));
  } catch(err){res.status(500).json({error:err.message});}
});
router.patch('/operational-goals/:id', requireAuth, requireRole('equity_lead'), async (req, res) => {
  try {
    ...
    saveDb(); res.json(dbRow(db,'SELECT * FROM operational_goals WHERE id=?',[req.params.id]));
  } catch(err){res.status(500).json({error:err.message});}
});

// Agents & Learning Loop
router.get('/agent-definitions', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM agent_definitions WHERE is_active=1 ORDER BY name ASC')); } catch(err){res.status(500).json({error:err.message});}
});
router.get('/learning-loop-proposals', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM learning_loop_proposals ORDER BY created_at DESC')); } catch(err){res.status(500).json({error:err.message});}
});
router.post('/learning-loop-proposals/:id/decide', requireAuth, requireRole('equity_lead'), async (req, res) => {
  try {
    ...
    saveDb(); res.json(dbRow(db,'SELECT * FROM learning_loop_proposals WHERE id=?',[req.params.id]));
  } catch(err){res.status(500).json({error:err.message});}
});

// Approval Queue
router.get('/approval-queue', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,"SELECT * FROM approval_queue WHERE status='pending' ORDER BY created_at ASC")); } catch(err){res.status(500).json({error:err.message});}
});
router.post('/approval-queue/:id/decide', requireAuth, requireRole('equity_lead'), async (req, res) => {
  try {
    ...
    saveDb(); res.json(dbRow(db,'SELECT * FROM approval_queue WHERE id=?',[req.params.id]));
  } catch(err){res.status(500).json({error:err.message});}
});
...
router.get('/consultant-documents', requireAuth, requireRole('equity_lead'), async (req, res) => {
  try {
    ...
    res.json(dbRows(db,sql+' ORDER BY created_at DESC',p));
  } catch(err){res.status(500).json({error:err.message});}
});
router.post('/consultant-documents', requireAuth, requireRole('equity_lead'), async (req, res) => {
  try {
    ...
    saveDb(); res.status(201).json(dbRow(db,'SELECT * FROM consultant_documents WHERE id=?',[id]));
  } catch(err){res.status(500).json({error:err.message});}
});
...
router.get('/audit-log', requireAuth, requireRole('equity_lead'), async (req, res) => {
  try {
    ...
    res.json(dbRows(db,sql+' ORDER BY created_at DESC LIMIT ?',[...p,limit]));
  } catch(err){res.status(500).json({error:err.message});}
});

// ODET Hub
router.get('/working-groups', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM working_groups ORDER BY name ASC')); } catch(err){res.status(500).json({error:err.message});}
});
router.get('/odet-records', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM odet_records ORDER BY created_at DESC LIMIT 50')); } catch(err){res.status(500).json({error:err.message});}
});
router.get('/equity-team', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM equity_team ORDER BY full_name ASC')); } catch(err){res.status(500).json({error:err.message});}
});
router.get('/community-feedback', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM community_feedback ORDER BY created_at DESC LIMIT 50')); } catch(err){res.status(500).json({error:err.message});}
});
router.post('/community-feedback', requireAuth, async (req, res) => {
  try {
    ...
    saveDb(); res.status(201).json(dbRow(db,'SELECT * FROM community_feedback WHERE id=?',[id]));
  } catch(err){res.status(500).json({error:err.message});}
});
router.get('/calendar-events', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,"SELECT * FROM calendar_events WHERE event_date >= date('now','-7 days') ORDER BY event_date ASC LIMIT 30")); } catch(err){res.status(500).json({error:err.message});}
});
...
router.get('/cos/clusters', requireAuth, async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,'SELECT * FROM cos_clusters ORDER BY cluster_id ASC')); } catch(err){res.status(500).json({error:err.message});}
});
router.get('/cos/atoms', requireAuth, async (req, res) => {
  try {
    ...
    res.json(dbRows(db,sql+' ORDER BY a.atom_id ASC',p));
  } catch(err){res.status(500).json({error:err.message});}
});
router.patch('/cos/atoms/:id/toggle', requireAuth, requireRole('equity_lead'), async (req, res) => {
  try {
    ...
    saveDb(); res.json(dbRow(db,'SELECT * FROM cos_atoms WHERE id=?',[req.params.id]));
  } catch(err){res.status(500).json({error:err.message});}
});
...
router.get('/cos/approvals', requireAuth, requireRole('equity_lead'), async (req, res) => {
  try { const db=await getDb(); res.json(dbRows(db,"SELECT * FROM cos_approvals WHERE status='pending' ORDER BY created_at ASC")); } catch(err){res.status(500).json({error:err.message});}
});
router.post('/cos/approvals/:id/decide', requireAuth, requireRole('equity_lead'), async (req, res) => {
  try {
    ...
    res.json(dbRow(db,'SELECT * FROM cos_approvals WHERE id=?',[req.params.id]));
  } catch(err){res.status(500).json({error:err.message});}
});
router.get('/cos/outputs', requireAuth, requireRole('equity_lead'), async (req, res) => {
  try {
    ...
    res.json(dbRows(db,sql+' ORDER BY created_at DESC LIMIT 50',p));
  } catch(err){res.status(500).json({error:err.message});}
});
